 ...
 * @author J³
 */
@WebServlet(name = "InserirReferee", urlPatterns = {"/CRUDReferee/Inserir"})
public class Inserir extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (daoReferee.obter(id) == null) {
            Referee referee = new Referee();
            referee.setPlayerIdPlayer(id);
            referee.setCatRefereeIdReferee(catReferee);

            daoReferee.inserir(referee);
            request.setAttribute("message", "Arbitro " + new DAOPlayer().obter(referee.getPlayerIdPlayer()).getNamePlayer() + ": " + referee.getCatRefereeIdReferee().getNameCatReferee() + " inserido com sucesso");

        } else {
            request.setAttribute("message", "Arbitro " + id + " já cadastrado!");
        }
